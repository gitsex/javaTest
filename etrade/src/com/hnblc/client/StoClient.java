...
public class StoClient {
	...
	public  static String doGet(String orderNumber) throws UnsupportedEncodingException  
	{
		 ...
		 List<Map<String, String>> Order =DBsql.getOrderList("where orderNumber='"+orderNumber+"' Limit 1");
		   if(Order.size()>0)
		   {
		    ...
		    String Remark=null;
		   try {
				String xml =StoClient.setXml(Order,batchNumber);
				if(xml.equals(""))
				{
				...
				String data_digest =getDataDigest(xml);
				String billinfo_interface = getEncryptXmlStr(xml);	
				Map<String, String> Send = new HashMap<String, String>();  
				Send.put("data_digest", data_digest);
				Send.put("billinfo_interface", billinfo_interface);
				
			    String ret =Curl.stPost(Send,orderNumber,batchNumber);    
			    //ret= new String(ret.getBytes("GBK"),"UTF-8");   
			    Map<String, String> statusxml= XML.getStatus(ret.toString());

				String status = statusxml.get("Status");//XML.parseXml("Result","Status","ST/"+batchNumber+"/Return"+orderNumber+".xml");
				String logisticsNo = statusxml.get("logisticsNo");//XML.parseXml("Result","logisticsNo","ST/"+batchNumber+"/Return"+orderNumber+".xml");
				...
				Map<String, String> update= new HashMap<String,String>();
				 ...
				 DBsql.updateOrder(update,orderNumber);
							...
							{
								 //String logisticsNo = XML.parseXml(ret,"Result","logisticsNo",orderNumber);
	...
	public static String setXml(List<Map<String, String>> Order,String batchNumber) throws Exception {
