...
public class CopyOfProcess {
	...
	public static void main(String[] args) {
		...
		String batchNumber ="TG20151127020";
...
//		
		// 圆通推送
//		List<Map<String, String>> Order =DBsql.getOrderList("where is_lock=0 AND Ismatch=1 AND batchNumber='"+batchNumber+"' AND  logisticsExchangeStatus=0 AND orderStatus='已确认' ORDER BY id DESC");
		 Order =DBsql.getOrderList("where is_lock=0 AND Ismatch=1 AND batchNumber='"+batchNumber+"' AND  logisticsExchangeStatus=0 AND orderStatus='已确认' ORDER BY id DESC");
//		  
		for(int i=0;i<Order.size();i++)
		{
			 Map<String, String> item = Order.get(i);  
