...
public class CopyOfOrderClient {

	public static  Map<String, String> account;
	
	public static void doSend(String orderNumber) throws Exception {
	  ...
	  // 准备订单信息
		String dataInfo = getOrderXmlStr(orderNumber);
		if(dataInfo.equals(""))
		{
			...
			// RSA加密
			String encryptDataInfo = EncreptAndDecreptUtil.encreptData(dataInfo,account.get("publicKey"));
			...
			// 生成请求报文
			String xml = generateXml(encryptDataInfo, signature,item.get("IsSend"));
			...
			//调用webservice服务 http://218.28.185.212:9090  http://192.168.11.123:8080
			
			String PostResult = sendHttpPOSTRequest("http://218.28.185.212:9090/BIService/service/order/pushOrder",map,orderNumber,batchNumber,item.get("shopName"));
			// 查看响应回执
...
//		xmlSb.append("\t\t<Key>"+account.get("publicKey")+"</Key>\n");  //companyKey为全局变量
		if("京东:普丽普莱海外旗舰店".equalsIgnoreCase(account.get("shopName"))){
			xmlSb.append("\t\t<Key>"+account.get("companyKey")+"</Key>\n");  //companyKey为全局变量
		}else{
			xmlSb.append("\t\t<Key>"+account.get("publicKey")+"</Key>\n");  //companyKey为全局变量
		}
...
//		xmlSb.append("\t\t<CreatTime>2015-04-21 12:12:56</CreatTime>\n");
	...
	public static String getOrderXmlStr(String orderNumber) {
