	 ...
	 public static  Map<String, String> getStatus(String xml) throws DocumentException {
			...
			try {
			     ...
			     org.dom4j.Element element;
					...
					for (Iterator<?> k = root.elementIterator("Header"); k.hasNext();) {
						
						                     element=  (org.dom4j.Element) k.next();
						                   Status.put("Status",element.elementText("success"));
						                   System.out.println(element.elementText("success"));
					            
					  } 
					
					for (Iterator<?> k = root.elementIterator("ResponseOrder"); k.hasNext();) {
						
				                   element=  (org.dom4j.Element) k.next();
				       
				                   Status.put("Remark",element.elementText("ErrMsg"));
				                   System.out.println(element.elementText("Remark"));
				                   Status.put("logisticsNo",element.elementText("OrderID"));
